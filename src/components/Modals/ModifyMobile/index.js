      ...
      let type = userInfo.isGoogleBind && userInfo.isOpenGoogle ? 2 : userInfo.isMobileBind ? 0 : 1;
        ...
        return(
          <div className='ModifyMobile'>
            {
              isShow?
                <Modal title='换绑手机' okText='确认绑定' next={0} onOk={this._ok} onClose={this._onClose}>
                  <div style={{ padding: '0 125px', marginTop: 40, marginBottom: 60 }}>
                    <div style={{ display: 'flex', marginBottom: 20 }} className='hasSelect'>
                      ...
                      />
                      <LineInput name='newMobile' placeholder='新手机号码' onChange={this.handleUserInput} propsStyle={{ flex: 1 }} validate='number'/>
                    </div>
                    <div style={{ marginBottom: 20, display: 'flex'}}>
                      <LineInput placeholder='新手机验证码' type='text' name='newMobileCode' onChange={this.handleUserInput} propsStyle={{ flex: 1 }} validate='yzm'/>
                      <TimeButton  radius={true} onClick={this._sendNewSmsMobile} ref={ele => this.timePN = ele}/>
                    </div>
                    {
                      type == 0?
                        <div style={{ marginBottom: 20, display: 'flex' }}>
                          <LineInput placeholder='旧手机验证码' type='text' name='mobileCode' propsStyle={{flex: 1}} onChange={this.handleUserInput} validate='yzm'/>
